...
def tempo_warp(wav: torch.Tensor, sr: int, rate: float) -> torch.Tensor:
    ...
    Tries sox first; falls back to a phase-vocoder time stretch because
    torchaudio's sox bindings are deprecated and absent from many recent
    builds (including newer Colab images) -- without this fallback, every
    bass/other stem would fail during pair ingest and silently burn the
    Demucs+MERT compute already spent on the track."""
    try:
...
def make_rhythm_positive(drums: torch.Tensor, sr: int, seconds: float = 10.0) -> torch.Tensor:
    """A different section of the same drum stem -> same groove/kit identity.

    NOTE: the previous version returned the drums bit-identical (positive ==
    anchor, so the head learned no invariance and triplet accuracy read as
    misleadingly perfect) while also running a pitch_shift on the pitched
    stems whose result was thrown away -- pure wasted compute inside the GPU
    ingest loop. Segment-based positives fix both."""
